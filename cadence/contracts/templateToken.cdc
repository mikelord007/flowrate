...
access(all) contract templateToken: FungibleToken {

    /// The event that is emitted when new tokens are minted
    access(all) event TokensMinted(amount: UFix64, type: String)

    /// Total supply of templateTokens in existence
        ...
        switch viewType {
            ...
            case Type<FungibleTokenMetadataViews.FTVaultData>():
                return FungibleTokenMetadataViews.FTVaultData(
                    ...
                    receiverLinkedType: Type<&templateToken.Vault>(),
                    metadataLinkedType: Type<&templateToken.Vault>(),
                    createEmptyVaultFunction: (fun(): @{FungibleToken.Vault} {
                        return <-templateToken.createEmptyVault(vaultType: Type<@templateToken.Vault>())
                    })
                )
            case Type<FungibleTokenMetadataViews.TotalSupply>():
                return FungibleTokenMetadataViews.TotalSupply(
                    totalSupply: templateToken.totalSupply
                )
        }
        return nil
    }
    ...
    access(all) resource Vault: FungibleToken.Vault {
        ...
        access(contract) fun burnCallback() {
            if self.balance > 0.0 {
                templateToken.totalSupply = templateToken.totalSupply - self.balance
            }
            self.balance = 0.0
        }
        ...
        access(all) view fun getViews(): [Type] {
            return templateToken.getContractViews(resourceType: nil)
        }

        access(all) fun resolveView(_ view: Type): AnyStruct? {
            return templateToken.resolveContractView(resourceType: nil, viewType: view)
        }
        ...
        /// elsewhere.
        ///
        access(FungibleToken.Withdraw) fun withdraw(amount: UFix64): @templateToken.Vault {
        ...
        access(all) fun deposit(from: @{FungibleToken.Vault}) {
            let vault <- from as! @templateToken.Vault
        ...
        ///
        access(all) fun createEmptyVault(): @templateToken.Vault {
    ...
    /// account to be able to receive deposits of this token type.
    ///
    access(all) fun createEmptyVault(vaultType: Type): @templateToken.Vault {
        return <- create Vault(balance: 0.0)
    }

    access(all) fun mintTokens(amount: UFix64): @templateToken.Vault {
        templateToken.totalSupply = templateToken.totalSupply + amount
        emit TokensMinted(amount: amount, type: templateToken.getType().identifier)
        return <-create Vault(balance: amount)
    }
    ...
    init() {
        ...
        //
        let exampleTokenCap = self.account.capabilities.storage.issue<&templateToken.Vault>(self.VaultStoragePath)
        self.account.capabilities.publish(exampleTokenCap, at: self.VaultPublicPath)
        let receiverCap = self.account.capabilities.storage.issue<&templateToken.Vault>(self.VaultStoragePath)
